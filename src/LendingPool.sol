...
/**
 * @title LendingPool
 * @notice Core capital management hub - manages total pool, tracks lender shares, coordinates lending
 * @dev Central contract for all capital operations
 */
contract LendingPool is ILendingPool, ERC20, Ownable, ReentrancyGuard {
     ...
     * @dev Automatically applies penalty if before lock period
     */
    function withdrawPartial(uint256 usdcAmount) external nonReentrant {
        ...
        if (isEarlyWithdrawal) {
            // Calculate penalty
            uint256 daysRemaining = (lenderInfo.lockUntil - block.timestamp) / SECONDS_PER_DAY;
            uint256 penaltyPercent = daysRemaining * PRECISION; // 1% per day
            if (penaltyPercent > 50 * PRECISION) {
                penaltyPercent = 50 * PRECISION; // Max 50%
            }

            // Calculate penalty on requested amount
            penaltyAmount = (usdcAmount * penaltyPercent) / PERCENT_100;
            
            // Split penalty: 50% to protocol, 50% stays in pool
            uint256 protocolShare = penaltyAmount / 2;
            uint256 lenderShare = penaltyAmount - protocolShare;
            
            // Net amount lender receives
            actualWithdrawal = usdcAmount - penaltyAmount;
            
            // Calculate shares to burn based on gross amount (before penalty)
            sharesToBurn = (usdcAmount * PRECISION) / currentSharePrice;
            
            require(usdcAmount <= availableCash, "Insufficient liquidity");
            
            // Update state
            availableCash -= (usdcAmount - lenderShare); // lenderShare stays in pool
            reserveFund += protocolShare;
            
        } else {
            // Normal withdrawal - no penalty
            actualWithdrawal = usdcAmount;
            sharesToBurn = (usdcAmount * PRECISION) / currentSharePrice;
        ...
        if (isEarlyWithdrawal) {
            emit WithdrawEarly(msg.sender, actualWithdrawal, penaltyAmount);
        } else {
    ...
    /**
     * @notice Early withdrawal with penalty (50/50 split: protocol + remaining lenders)
     * @dev Penalty is split: 50% to protocol reserve, 50% stays in pool (increases share price)
     */
    function withdrawEarly() external override nonReentrant {
        ...
        // Calculate penalty
        uint256 daysRemaining = (lenderInfo.lockUntil - block.timestamp) /
            SECONDS_PER_DAY;
        uint256 penaltyPercent = daysRemaining * PRECISION; // 1% per day (6 decimals)
        if (penaltyPercent > 50 * PRECISION) {
            penaltyPercent = 50 * PRECISION; // Max 50%
        }

        // Calculate gross withdrawal value
        // shares (6 decimals) * sharePrice (6 decimals) / PRECISION = USDC value (6 decimals)
        uint256 grossAmount = (lenderInfo.shares * getSharePrice()) / PRECISION;

        // Calculate penalty amount
        // grossAmount (6 decimals) * penaltyPercent (6 decimals) / PERCENT_100 = penalty (6 decimals)
        uint256 totalPenalty = (grossAmount * penaltyPercent) / PERCENT_100;

        // Split penalty: 50% to protocol, 50% to remaining lenders (stays in pool)
        uint256 protocolShare = totalPenalty / 2;
        uint256 lenderShare = totalPenalty - protocolShare; // Use subtraction to avoid rounding issues

        // Net amount lender receives (USDC in 6 decimals)
        uint256 netAmount = grossAmount - totalPenalty;
        ...
        // Update state - 50/50 penalty split
        // Only remove (grossAmount - lenderShare) from availableCash
        // lenderShare stays in pool, automatically increases share price for remaining lenders
        availableCash -= (grossAmount - lenderShare);
        reserveFund += protocolShare; // Protocol gets 50% of penalty
        delete lenders[msg.sender];

        // Transfer net amount to lender (gross - total penalty)
        require(USDC.transfer(msg.sender, netAmount), "Transfer failed");

        emit WithdrawEarly(msg.sender, netAmount, totalPenalty);
    }
